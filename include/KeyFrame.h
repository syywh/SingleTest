...
public:
    ...
    KeyFrame(cv::Mat tim, vector<cv::KeyPoint> tkeypoints, cv::Mat tDescriptors);
//     KeyFrame(vector<cv::KeyPoint> keypoint,cv::Mat descriptor, std::vector<ORB_SLAM::MapPoint*> mapoints,long unsigned int mid);///////
...
//     vector<pair<long unsigned int, list<pair<float,ModelFrame*> > > > patch_Model;
    vector<pair<int, long unsigned int> > Object_Model;
//     vector<pair<int, float> > Object_Model_match_score;
//     vector<pair<int, float> > Refined_Object_Model_match_score;
//     vector<pair<int, long unsigned int> > Refined_Object_Model;
//     vector<pair<cv::KeyPoint,long unsigned int > > patch_bestMatch;	//可能可以换成指针
    vector<pair<int,  map<float, ModelFrame*> > > Match_map_result;
    vector<pair<int,  map<float, ModelFrame*> > > Refine_Match_map_result;
    set<ModelFrame* > hasObject;
    set<int > ObjectName;
    vector<pair<int,vector<cv::KeyPoint> > > CheckedByFundamental;
    ...
    std::vector<MapPoint*> mvpMapPoints;///////////////原来是protected
