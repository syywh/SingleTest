...
public:
    ...
    // Used to track the local map (Tracking)		//模型  keyframe
    int SearchByProjection(Frame &F, const std::vector<MapPoint*> &vpMapPoints, const float th=3);

    // Project MapPoints tracked in last frame into the current frame and search matches.
    // Used to track from previous frame (Tracking)		//模型 keyframe
    int SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, float th);

    // Project MapPoints seen in KeyFrame into the Frame and search matches.
    // Used in relocalisation (Tracking)	//将观测的地图点投影到新的frame，match
    ...
    // Used in loop detection (Loop Closing)		//模型投影回keyframe的时候用		//用Similarity 变换投影地图点，算匹配
    ...
    // Project MapPoints into KeyFrame and search for duplicated MapPoints.		//将地图点投影到keyframe，搜索duplicated 地图点
