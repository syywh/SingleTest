    ...
    mvInvLevelSigma2(F.mvInvLevelSigma2), mpMap(pMap)
{
    ...
    SetPose(F.mTcw);    
//     all.reserve(3000*sizeof(int));
    all.resize(mvKeys.size());
    segment.reserve(3000*sizeof(int));
    tsegment.reserve(3000*sizeof(int));
    
}

KeyFrame::KeyFrame(cv::Mat tim, vector< cv::KeyPoint > tkeypoints, cv::Mat tDescriptors):mvKeys(tkeypoints),mDescriptors(tDescriptors)
{
  tim.copyTo(im);
      all.resize(mvKeys.size());
    segment.reserve(3000*sizeof(int));
