...
void LocalMapping::SetObjectRecognizer(ObjectRecognition* pObjectRecognizer)
{
  mpObjectRecognizer = pObjectRecognizer;
}
...
void LocalMapping::Run()
{
    ...
    while(ros::ok())
    {
	    ...
	    mpObjectRecognizer->InsertKeyFrame(mpCurrentKeyFrame);
        }
...
void LocalMapping::InsertModelPoints(vector<vector<cv::Point3f> >  maddModelPoints)
{
  boost::mutex::scoped_lock lock(mMutexModelPoints);
  for(size_t i=0; i<maddModelPoints.size(); i++)
  {
    mlvModelPoints3f.push_back(maddModelPoints[i]);
  }
