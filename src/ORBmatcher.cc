...
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)	//用BoW匹配，很快！
{
    vector<MapPoint*> vpMapPointsKF = pKF->GetMapPointMatches();	// return mvpMapPoints;

    vpMapPointMatches = vector<MapPoint*>(F.mvpMapPoints.size(),static_cast<MapPoint*>(NULL));

    DBoW2::FeatureVector vFeatVecKF = pKF->GetFeatureVector();//FeatureVector：map<Node,vector<id>>
    ...
    while(KFit != KFend && Fit != Fend)
    {
        ...
        {
            vector<unsigned int> vIndicesKF = KFit->second;	//节点上的特征点
